...
This plan records the product delivered in `4.0.0` through [smtp-connection-pool #10](https://github.com/simple-java-mail/smtp-connection-pool/issues/10). Phases 0–6 are complete; the remaining unchecked items belong to the separate Simple Java Mail follow-up tracked in [#698](https://github.com/bbottema/simple-java-mail/issues/698). [Simple Java Mail #699](https://github.com/bbottema/simple-java-mail/issues/699) is a related physical-transport initiative, not a dependency.
...
The demo is built and tested with the rest of the project and is part of every release check, but it is not published. The missing standalone `batch-module` example is deliberate until Simple Java Mail 10.0.0 delivers #698's supported public API.

## Delivery target

- Release: `4.0.0` (major milestone release signaling the project's expansion into a broader SMTP integration platform).
- Release contents: preserved original pool module, optional Jakarta Mail provider module, and optional Camel adapter module.
- Release order: publish and verify `smtp-connection-pool 4.0.0` first; update Simple Java Mail afterwards.
...
Boundary for the later Simple Java Mail work:
...
- Simple Java Mail #698 must expose only Jakarta Mail abstractions and must not make its public batch API or receipt behavior depend on Angus. Any new transport-neutral receipt adapter required by #699 belongs to the Simple Java Mail plan, not this library.

## Phase 0: freeze the public contracts

- [x] Convert the repository to a Maven reactor with a parent/aggregator POM.
- [x] Move the current code into its own module without changing the existing `org.simplejavamail:smtp-connection-pool` dependency declaration.
...
- [x] Confirm the Java baseline for the original pool and Jakarta provider; keep Camel's Java/Camel constraints isolated in its own module.
...
- [x] Add a separate CircleCI job that compiles and tests the original pool plus Jakarta provider on an actual JDK 8, and keep its build-time plugins runnable on Java 8.
...
- [x] Add a checksum-pinned japicmp comparison against the published `3.1.0` library and keep publishing it as `org.simplejavamail:smtp-connection-pool`, with the same packages, public API, automatic-module name, OSGi identity, Java 8 bytecode, and dependency ranges.
...
## Phase 1: add the exclusive transport lease
...
- [x] Keep lease and allocator APIs expressed only in Jakarta Mail and pool-library types; no physical-provider implementation appears in a public signature.
...
- [x] Add a non-published `smtp-connection-pool-demo` project that is built with the rest of the repository and contains a random-port Wiser SMTP server plus executable `main` classes.
- [x] Demonstrate direct pool usage first, Simple Java Mail as a higher-level library built directly on the pool second, and plain Jakarta Mail/Spring/Camel as path-3 variants.
...
- [x] Defer the standalone path-2 demo until Simple Java Mail 10.0.0 publishes the supported #698 API.
...
- [x] Verify the complete candidate build with tests, Javadocs, SpotBugs, binary compatibility, artifact packaging, and the documented examples; repeat on the eventual reviewed release commit.
- [x] Inspect the combined effective POM and perform a `skipPublishing` deploy-lifecycle rehearsal; the shared parent plus original pool, provider, Camel, and demo outputs install locally with aligned metadata. Central staging remains publication-time verification because `skipPublishing` intentionally stages nothing.
- [x] During publication rehearsal, inspect the generated Central bundle and prove that it contains the parent plus all three published modules, but not `smtp-connection-pool-demo`. The local structural rehearsal skips signing because GPG is supplied by the CircleCI release context; the actual release bundle remains subject to Central's signature validation.
- [x] Merge to `master`; let CircleCI own version selection, release commit, and tag creation.
- [x] With explicit release approval, approve the **major** deployment job. The first attempt stopped before publication on a duplicate-workspace attachment; after isolating the Java 8 gate, the clean retry completed through the same major job.
- [x] Verify tag `4.0.0` and all three modules in Maven Central, including sources, Javadocs, POMs, and signatures; verify that the demo was not published.
...
This phase is not part of the completed `smtp-connection-pool 4.0.0` release. It is separate work owned by [Simple Java Mail #698](https://github.com/bbottema/simple-java-mail/issues/698), targeted at Simple Java Mail 10.0.0. That release may follow months later and does not block this project's demos or releases. [Simple Java Mail #699](https://github.com/bbottema/simple-java-mail/issues/699) is a design input: #698 must leave room for a transport-neutral implementation, but it does not implement #699.

- [ ] Upgrade Simple Java Mail to the released `smtp-connection-pool 4.0.0` dependency.
...
- Existing users keep the same `org.simplejavamail:smtp-connection-pool` dependency and direct API.
- Although `4.0.0` deliberately signals the architectural milestone, the original library remains binary-compatible with `3.1.0`; the major version is not permission to break existing users unnecessarily.
...
- The original pool and Jakarta provider stay compatible with Java 8 even though Camel requires Java 17.
- Simple Java Mail users retain current high-level behavior; the new batch facade is additive.

## `smtp-connection-pool 4.0.0` definition of done
...
- the original pool and Jakarta provider compile and test on JDK 8 independently of the complete JDK 21 verification;
- all published public APIs and properties are documented with available-version markers;
- the parent and all three published modules are present in Maven Central under one version, while the demo is absent;
- GitHub issue, milestone, tag, release, and release-note records agree; and
- Simple Java Mail #698 is linked and records released version `4.0.0` as its available prerequisite.
